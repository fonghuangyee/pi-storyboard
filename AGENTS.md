...
- Do not add network calls, subprocesses, timers, background work, or session-file parsing. The explicit user-initiated storyboard settings command may atomically write only its validated dedicated `pi-storyboard.json` file when the user saves.
